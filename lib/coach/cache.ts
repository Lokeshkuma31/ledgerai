...
export function computeCoachSignature(
  ...
  return `${txParts.join("|")}::mem${memoryEntryCount}::budgets${budgetParts.join("|")}::recs${recommendationIds.join("|")}::merchants${merchantIds.join("|")}::recurring${recurringStatuses.join("|")}::forecast${forecastDay}::feed${feedItemIds.join("|")}::policy${policyCandidateIds.join("|")}::workflow${workflowRunIds.join("|")}::goals${goalProgressKeys.join("|")}::imports${importSummaryKeys.join("|")}`;
}
